...
class _UsageFinder(ast.NodeVisitor):
    ...
    def _add(self, node: ast.AST):
        ...
        ctx_line = (
            self.source_lines[line - 1].rstrip() if 0 < line <= len(self.source_lines) else ""
        )
        self.usages.append(
            Usage(
...
class CodeIntelligence:
    ...
    def project_summary(
        ...
        parse_errors = {rel(p): i.parse_error for p, i in project_intel.items() if i.parse_error}
